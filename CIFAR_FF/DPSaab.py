...
def noisycount(sensitivety,epsilon):
	...
	print(n_value)
	return n_value'''
def lmatch(a,sensitivty,epsilon):
	w_conv1Noise=np.random.laplace(0.0,(31*sensitivty)/(epsilon),75)
	w_conv1Noise=np.reshape(w_conv1Noise,75)
	T=a+w_conv1Noise
	print(w_conv1Noise)
	return T
def lmatch_1(a,sensitivty,epsilon,value):
	s={}
	for i in range(len(value)):
		s[i]=(value[i]/sum(value))*epsilon
	r = []
	for j in range(len(s)):
		w = np.random.laplace(0.0, sensitivty/ s[j], 75)
		w=np.reshape(w,75)
		r.append(w)
	u = np.array(r)
	print(u)
	t = u + a
	return t
def lmatch_2(a,sensitivty,epsilon,value):
	s={}
	value_1=value[::-1]
	for i in range(len(value_1)):
		s[i]=(value_1[i]/sum(value_1))*epsilon
	r = []
	for j in range(len(s)):
		w = np.random.laplace(0.0, sensitivty/ s[j], 75)
		w=np.reshape(w,75)
...
def find_kernels_pca(sample_patches, num_kernels, energy_percent):
	...
	dc_kernel=dc_kernel+np.random.laplace(0.0, dc_kernel[0][1]/ (epsilon-epsilon1), 75)
	print(dc_kernel[0][1])
	...
	return kernels, mean,feature_expectation
def find_kernels_pca_1(sample_patches, num_kernels, energy_percent):
	'''
	Do the PCA based on the provided samples.
	If num_kernels is not set, will use energy_percent.
	If neither is set, will preserve all kernels.
	...
	y=np.log(pca.explained_variance_[:5])
	print(y)
	y=np.delete(y,y.shape[0]-1,0)
	#num_channels=sample_patches.shape[-1]
	#largest_ev=[np.var(dc*np.sqrt(num_channels))]
...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes):
	...
	for i in range(num_layers):
		...
		if i==0:
		    kernels, mean,feature_expectation = find_kernels_pca_1(sample_patches, num_kernel, energy_percent)
		else:
			kernels, mean,feature_expectation = find_kernels_pca_1(sample_patches, num_kernel, energy_percent)
