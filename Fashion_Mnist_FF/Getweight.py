import Fashion_Mnist_FF.data as data
import pickle
import numpy as np
import sklearn
import time
#import cv2
import keras
import Fashion_Mnist_FF.saab as saab
...
def main():
	...
	for k in range(len(num_clusters)):
		if k!=len(num_clusters)-1:
			...
			report = sklearn.metrics.classification_report(pred_labels, train_labels)
			print(report)
			print('training acc is {}'.format(acc_train))
			print("------------------- End: train -> using %10f seconds" % (time.time() - t0))
	# save data
